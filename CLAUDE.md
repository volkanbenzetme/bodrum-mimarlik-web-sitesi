...
Real brand/project facts (services, the 7-project portfolio, the 8-step KAIRO Yöntemi, restoration credentials, contact info) came from the founder's own KAIRO portfolio PDF and tanıtım föyü, shared 2026-09-03 — treat those as source of truth over anything invented in earlier drafts of this site. The site now features all 7 real projects (added D-16 Renovasyon, Kadim Bey Müştemilat, and Fırat Bey Villa to the original 4 on 2026-09-03) with real AI-architectural-render photos for all 7 project cards (`site/images/`, added 2026-09-03) — the illustrative SVG sketches used earlier are gone. Note these are AI renders the founder supplied, not on-site photography of built work.
...
- `images/` — web-optimized JPGs used by `#projeler` (one hero photo per project card, `<img>` inside `.project-sketch`, `object-fit: cover`). Sourced from AI-architectural-render images the founder supplied in chat; resized to 1600px-wide JPG (quality ~78) via a PowerShell/System.Drawing script since no ImageMagick/Python is available on this machine — reuse that approach for future images rather than committing multi-MB originals.
...
**Live at https://kairomimarlik.com** (custom domain, connected 2026-09-03 — DNS at Turhost points 4 A records at GitHub Pages' IPs, `CNAME` file in this repo holds the domain, HTTPS is enforced with an approved cert). Also reachable at https://volkanbenzetme.github.io/bodrum-mimarlik-web-sitesi/ (GitHub Pages, repo is public) — same content, GitHub's default URL.

A `.nojekyll` file at repo root is required — without it GitHub Pages tries to run this static site through Jekyll and the build fails instantly (`duration: 0`, generic "Page build failed"). Don't remove it. `index.html` in this folder is the served copy — a plain-HTML version of `Main.dc.html` with the `{{accent}}` template placeholder resolved to a literal color and the `<x-dc>`/`<helmet>` wrapper flattened into a real `<head>`, since GitHub Pages has no Design Canvas runtime to do that. **There is no build step wiring these together — after editing `Main.dc.html`, manually re-apply the same two changes to `index.html` (or regenerate it) and commit both.**
